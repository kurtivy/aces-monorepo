...
export async function sellerVerificationRoutes(fastify: FastifyInstance) {
  const verificationService = new SellerVerificationService(fastify.prisma);
  const log = logger.child({ module: 'seller-verification-routes' });
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        const status = await verificationService.getVerificationStatus(request.user!.id);
        return reply.send({
          success: true,
          data: status,
        });
