// backend/src/api/submissions.ts - V1 Clean Implementation
...
const buildSubmissionsApp = async (): Promise<FastifyInstance> => {
  ...
  fastify.register(multipart, {
    limits: {
      fileSize: 10 * 1024 * 1024, // 10MB for submission images
    },
  });
...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  try {
    ...
    // Handle path rewriting: /api/v1/submissions/something → /something
