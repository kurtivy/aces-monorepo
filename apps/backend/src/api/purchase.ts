...
const buildPurchaseApp = async (): Promise<FastifyInstance> => {
...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  try {
    appPromise = appPromise ?? buildPurchaseApp();
    ...
    // Handle path rewriting: /api/v1/purchase/something → /something
    if (req.url?.startsWith('/api/v1/purchase')) {
      req.url = req.url.replace('/api/v1/purchase', '') || '/';
    }
