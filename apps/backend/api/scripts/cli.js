...
// Initialize services
const recoveryService = new recovery_service_1.RecoveryService(prisma);
const approvalService = new approval_service_1.ApprovalService(prisma);
const biddingService = new bidding_service_1.BiddingService(prisma);
async function main() {
...
Examples:
  ...
  pnpm cli stats
    ...
    try {
        ...
        logger_1.logger.error({ error, command, args }, 'CLI command failed');
...
async function handleRecoverTransaction(args) {
    ...
    try {
        const result = await recoveryService.resubmitTransaction(submissionId, 'CLI', `cli-recovery-${Date.now()}`);
        ...
        console.error(`❌ Failed to resubmit transaction: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleReplayWebhook(args) {
    ...
    try {
        const result = await recoveryService.replayWebhook(webhookLogId, 'CLI', `cli-webhook-replay-${Date.now()}`);
        if (result.processed) {
        ...
        console.error(`❌ Failed to replay webhook: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleStats() {
    ...
    try {
        ...
        if (recoveryStats.failedTransactions > 0) {
            console.log(`⚠️  Action Required: ${recoveryStats.failedTransactions} failed transactions need attention`);
        ...
        console.error(`❌ Failed to retrieve stats: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleListFailed() {
    ...
    try {
        ...
        console.error(`❌ Failed to list failed transactions: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleListWebhooks() {
    ...
    try {
        ...
        console.error(`❌ Failed to list webhooks: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleListPending() {
    ...
    try {
        ...
        console.error(`❌ Failed to list pending approvals: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
async function handleShowSubmission(args) {
    const [submissionId] = args;
    if (!submissionId) {
    ...
    try {
        const submission = (await approvalService.getSubmissionDetails(submissionId));
        if (!submission) {
        ...
        if (submission.bids && submission.bids.length > 0) {
            ...
            submission.bids.slice(0, 5).forEach((bid, index) => {
                console.log(`  ${index + 1}. ${bid.amount} ${bid.currency} by ${bid.bidder.walletAddress || bid.bidder.id}`);
            });
        ...
        if (submission.auditLogs && submission.auditLogs.length > 0) {
            ...
            submission.auditLogs.slice(0, 3).forEach((log, index) => {
                console.log(`  ${index + 1}. ${log.fromStatus || 'None'} → ${log.toStatus} (${log.actorType})`);
        ...
        console.error(`❌ Failed to get submission details: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
...
main().catch(async (error) => {
    logger_1.logger.error({ error }, 'Unhandled CLI error');
