...
var loggers = {
  request: /* @__PURE__ */ __name(
    (requestId, method, url, userAgent) =>
      logger.info(
        {
          type: 'request',
          requestId,
  ...
  response: /* @__PURE__ */ __name(
    (requestId, method, url, statusCode, responseTime) =>
      logger.info(
        {
          type: 'response',
          requestId,
  ...
  blockchain: /* @__PURE__ */ __name(
    (txHash, action, contractAddress) =>
      logger.info(
        {
          type: 'blockchain',
          txHash,
  ...
  database: /* @__PURE__ */ __name(
    (operation, table, recordId, duration) =>
      logger.info(
        {
          type: 'database',
          operation,
...
var errors = {
  unauthorized: /* @__PURE__ */ __name(
    (message) => (0, import_boom.unauthorized)(message || 'Unauthorized'),
    'unauthorized',
  ),
  forbidden: /* @__PURE__ */ __name(
    (message) => (0, import_boom.forbidden)(message || 'Forbidden'),
    'forbidden',
  ),
  notFound: /* @__PURE__ */ __name(
    (resource) => (0, import_boom.notFound)(resource ? `${resource} not found` : 'Not found'),
    'notFound',
  ),
  validation: /* @__PURE__ */ __name(
    (message, meta) => new AppError(message, 'VALIDATION_ERROR', meta),
    'validation',
  ),
  conflict: /* @__PURE__ */ __name((message) => (0, import_boom.conflict)(message), 'conflict'),
  badRequest: /* @__PURE__ */ __name(
    (message) => (0, import_boom.badRequest)(message),
    'badRequest',
  ),
  tooManyRequests: /* @__PURE__ */ __name(
    (message) => new import_boom.Boom(message, { statusCode: 429 }),
    'tooManyRequests',
  ),
    ...
    try {
      ...
      const dataWithSignedUrls = await Promise.all(
        data.map(async (submission) => ({
          ...submission,
          imageGallery: await ProductStorageService.convertToSignedUrls(submission.imageGallery),
        })),
      );
...
async function submissionRoutes(fastify) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        const { status, limit, cursor } = request.query;
        const result = await submissionService.getAllSubmissions(status ? { status } : void 0, {
          limit,
          cursor,
        });
        return reply.send({
...
var hasGoogleCloudCredentials2 = !!(
  process.env.GOOGLE_CLOUD_PROJECT_ID &&
...
var VisionService = class {
  ...
  static async downloadImageFromSecureStorage(imageUrl) {
    try {
      console.log('\u{1F4E5} Downloading from secure storage', {
        imageUrl,
        ...
        const verificationRecord = await tx.accountVerification.upsert({
          ...
          create: {
            ...
            documentAnalysisResults: data.documentAnalysisResults
              ? data.documentAnalysisResults
              : import_client2.Prisma.JsonNull,
          },
          update: {
            documentType: data.documentType,
            documentNumber: data.documentNumber,
      ...
      return {
        ...
        canSubmit: verification
          ? verification.attempts < 3 ||
            Date.now() - verification.lastAttemptAt.getTime() >= 24 * 60 * 60 * 1e3
          : true,
      };
    } catch (error) {
...
async function bidsRoutes(fastify) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        const { status, limit, cursor } = request.query;
        const result = await bidService.getUserBids(request.user.id, status ? { status } : void 0, {
          limit,
          cursor,
        });
        return reply.send({
      ...
      try {
        ...
        if (request.isMultipart()) {
          ...
          for await (const part of request.parts()) {
            console.log('\u{1F50D} Processing part:', {
              type: part.type,
  ...
  async generateDailyCandles(contractAddress) {
    try {
      ...
      for (const dayData of tokenDayData) {
        const netVolume = new import_decimal2.Decimal(dayData.tokensBought).minus(
          new import_decimal2.Decimal(dayData.tokensSold),
        );
        const totalVolume = new import_decimal2.Decimal(dayData.tokensBought).plus(
          new import_decimal2.Decimal(dayData.tokensSold),
        );
        ...
        const high = import_decimal2.Decimal.max(
          new import_decimal2.Decimal(open),
          new import_decimal2.Decimal(close),
        )
          .mul(1.05)
          .toString();
        const low = import_decimal2.Decimal.min(
          new import_decimal2.Decimal(open),
          new import_decimal2.Decimal(close),
        )
          .mul(0.95)
