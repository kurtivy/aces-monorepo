...
const NeonLogo3D: React.FC = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const normalizedX = (mouseX / centerX) * 0.3;
      const normalizedY = (mouseY / centerY) * 0.3;
  ...
  useEffect(() => {
    ...
    const allPaths: AllPathSets = {
      ...
      smileyFace: generatePathSet(
        logoSubPaths.smileyFace,
        "smileyFace",
        fadeInDuration
      ),
    };
    ...
    const phase1Start = 0;
    const phase2Start = phase1Start + outerCircleDuration * 1000;

    const timers = [
      setTimeout(() => setAnimationPhase(1), phase1Start),
      setTimeout(() => setAnimationPhase(2), phase2Start),
    ];
      ...
      <div
        ...
        style={{
          transform: `perspective(1000px) rotateX(${
            mousePosition.y * 20
          }deg) rotateY(${mousePosition.x * 20}deg) translateZ(${
            Math.abs(mousePosition.x * mousePosition.y) * 30
          }px)`,
        }}
      >
        <svg
