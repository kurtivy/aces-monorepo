...
export default function RWAForum() {
    ...
    setComments((prevComments) => {
      ...
      if (commentIndex !== -1) {
        current[commentIndex] = {
          ...current[commentIndex],
          likes: current[commentIndex].isLiked ? current[commentIndex].likes - 1 : current[commentIndex].likes + 1,
  ...
  const renderComment = (comment: Comment, depth = 0, path: number[] = []) => (
    <div key={comment.id} className={`${depth > 0 ? "ml-8 border-l border-[#D0B284]/20 pl-4" : ""}`}>
  ...
  return (
    <div className="h-[calc(100vh-300px)] flex flex-col bg-black rounded-lg">
      ...
      <div className="flex-shrink-0 p-4 border-b border-[#D0B284]/20">
        <h3 className="text-white text-lg font-semibold">Discussion ({comments.length} comments)</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-[#D0B284] scrollbar-track-[#231F20] min-h-0">
        <div className="space-y-4">{comments.map((comment, index) => renderComment(comment, 0, [index]))}</div>
      </div>
