...
import AcesOfBaseModal from './aces-of-base-modal';

const UnifiedInfoPillComponent: React.FC = () => {
  const [copied, setCopied] = useState(false);
  const [isVisible, setIsVisible] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeSection, setActiveSection] = useState<'contract' | 'base'>('contract');
  ...
  useEffect(() => {
    // Show overlay after a delay to match other UI elements
  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="text-xs font-medium text-[#D0B264] whitespace-nowrap">ACES:</span>
...
const UnifiedInfoPill = memo(UnifiedInfoPillComponent);

export default UnifiedInfoPill;
